...
class DualEncoderDecoder(nn.Module):
    ...
    def forward(self, obs, x_tar, isPair, p=0):
        ...
        output1, output2 = torch.zeros(0), torch.zeros(0)
        if isPair:
...
def loss(output, targets, d_y1, a1, a2, isPair):

    # a1, a2 = normalize_latent_space(a1), normalize_latent_space(a2)

    ## contrastive loss
    distance = F.pairwise_distance(a1, a2, p=2)
    margin = 10
    contrastive_loss = torch.mean((isPair) * distance**2 + (1-isPair) * torch.clamp(margin - distance, min=0)**2)

    log_loss = 0
    if isPair:
        log_loss = log_prob_loss(output, targets, d_y1) 

    lambda1 = 1

    return lambda1 * contrastive_loss + log_loss
...
def get_training_sample(validation_indices, X1, Y1, X2, Y2, OBS_MAX, d_N, d_x, d_y1, d_y2, time_len):
    ...
    d2 = d + 1 if d < d_N - 1 else d - 1
    p = np.random.rand(1) # random time point
    isPair = False

    PROBABILITY_OF_PAIR = 1.0
    if p<PROBABILITY_OF_PAIR:
        isPair = True
        d2 = d
    ...
    observations[:,2*d_x+d_y1:] = Y2[d2, perm[n:2*n]]
        
    perm = np.random.permutation(time_len)
    m = np.random.randint(0, 1) + 1 # number of targets
    target_X = X1[0, perm[:m]]
    target_Y1 = Y1[d, perm[:m]].reshape(1,-1)
    target_Y2 = Y2[d2, perm[:m]].reshape(1,-1)

    return torch.from_numpy(observations), torch.from_numpy(target_X), [torch.from_numpy(target_Y1), torch.from_numpy(target_Y2)], isPair
...
def compute_custom_loss(a1, a2):
        ## normalize a1 and a2 between -1 and 1
...
def normalize_latent_space(latent_space):
    # between -1 and 1
    latent_space = (latent_space - torch.mean(latent_space)) / torch.std(latent_space)
    return latent_space
